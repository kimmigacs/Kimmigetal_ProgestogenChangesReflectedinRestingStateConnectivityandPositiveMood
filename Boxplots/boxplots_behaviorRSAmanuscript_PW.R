...
ERtA_graph <- 
  ggboxplot(aa,
            x = "Group_deltaN", y = "behaviordelta_data", 
            color = "Group_deltaN", fill = "Group_deltaN",
            add = "jitter", 
            add.params = list(size = 2, alpha = 0.5, shape = 21, position = position_jitterdodge(jitter.height = 0, jitter.width = jitt))) +
  # Add mean
  stat_summary(fun = mean, geom = "point", shape = 23, size = 3, fill = "white", color = "black", position = position_dodge(0.25)) +
  # Add p-values using stat_pvalue_manual
#  stat_pvalue_manual(res[res$p.adj.signif != "ns", ], label = "p.adj.signif",
#                     y.position = c(0.95, 0.8, 0.8) + 0.2, tip.length = 0,
#                     bracket.shorten = 0.1, size = ssize, vjust = sva) +
#  stat_pvalue_manual(res.single[res.single$p.adj.signif != "ns", ], 
#                     label = "p.adj.signif", label.size = ssize,
#                     y.position = -1.40, vjust = 2.5 * sva,
#                     tip.length = 0, remove.bracket = TRUE) +
  # Add light grey horizontal line at y = 0
...
## save plot
hi <- 3
png("BehaviourGroupDiffBoxplot.png", units="in", width=10, height=3, res=300)
...
#source("boxplots_behaviorRSAmanuscript_PW.R")
